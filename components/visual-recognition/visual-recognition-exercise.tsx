...
interface VisualRecognitionExerciseProps extends VisualRecognitionConfig {}
...
export function VisualRecognitionExercise({
  imagesPerQuestion,
  correctImagesCount,
  timeLimit,
  categories,
  showImageNames,
  totalQuestions,
}: VisualRecognitionExerciseProps) {
  ...
  function selectImagesForQuestion(): {
    ...
    const targetCategory = categories[Math.floor(Math.random() * categories.length)] as ImageCategory

    // Get correct images from target category
    const targetCategoryImages = imageCategories[targetCategory]
    const shuffledTargetImages = [...targetCategoryImages].sort(() => 0.5 - Math.random())
    const correctImages = shuffledTargetImages.slice(0, correctImagesCount)

    // Get distractor images from other categories
    const distractorCategories = categories.filter((cat) => cat !== targetCategory) as ImageCategory[]
    ...
    const imagesPerDistractorCategory = Math.ceil(distractorImagesNeeded / distractorCategories.length)
    ...
    distractorCategories.forEach((category) => {
      ...
      const selectedFromCategory = shuffledCategoryImages.slice(0, imagesPerDistractorCategory)
  ...
  function setupNewQuestion() {
    ...
    const { targetCategory, displayedImages, correctImageIds } = selectImagesForQuestion()
      ...
      {exerciseState === "finished" ? (
        ...
        <>
          <div className="text-center mb-4">
            <h2 className="text-2xl font-bold mb-2">Ejercicio de Reconocimiento Visual</h2>
            <p className="text-gray-600">Selecciona todas las imágenes que pertenecen a la categoría indicada</p>
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="mt-4">
                  <Badge variant="outline" className="text-lg px-4 py-2">
                    Busca: {categoryLabels[questionState.targetCategory]}
                  </Badge>
                </div>
                {!showImageNames && (
                  <div className="mt-2">
                        ...
                        >
                          ...
                          {showImageNames && (
                            <div className="p-3 text-center">
                              <p className="text-sm font-medium text-muted-foreground truncate">{image.name}</p>
                            </div>
                          )}
                        </div>
                      )
