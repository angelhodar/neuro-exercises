...
const mediaMetadataSchema = z.object({
  name: z.string().describe("Un nombre corto y descriptivo de la entidad o entidades que se ven en la imagen."),
  description: z.string().describe("Una descripción corta de una sola frase sobre la entidad o entidades que se ven en la imagen. Evita empezar utilizando palabras como 'Un' o 'Una'."),
  tags: z.array(z.string()).max(5).describe("Una lista de máximo 5 etiquetas que clasifican la imagen (p. ej., 'animal', 'ropa', 'comida', 'felino', 'frutas')."),
});
...
async function main() {
  ...
  if (mediaToUpdate.length === 0) {
    console.log("✅ No media to update. All items have names and descriptions.");
    return;
  }
  ...
  try {
    ...
    if (generatedMetadata.length !== mediaToUpdate.length) {
      throw new Error(`AI returned ${generatedMetadata.length} items, but ${mediaToUpdate.length} were expected.`);
    }

    console.log(`🤖 AI Generated metadata for ${generatedMetadata.length} items.`);
